...
public class OrdersServiceImpl implements OrdersService {
    @Override
    public List getDistinct(List<Orders> ordersList) {
        List distinctOrder = new ArrayList<Orders>();
        List<String> keyList = new ArrayList<String>();
        String key ;
        for (Orders order:
                ordersList ) {
            key = order.getBranchCode()+order.getBusinessDate().getTime()+order.getOrderNo();
