...
//    	applicationSettingDao.getApplicationLock(scheduleJob.getPollSchemeType().name());
        ...
        if (schemeJobLog == null) {
            ...
            if (schemeJobLog.getStatus() == JobProcessStatus.PROGRESS) {
				if(currentTime - schemeJobLog.getLastUpdateTime().getTime() > otherTaskTimeout) {
					LogUtils.printLog(logger, "scheduleJob={}  PROGRESS Continue > {}", scheduleJob.getId(), otherTaskTimeout);
...
//					schemeJobLog.setLastJobInd(LatestJobInd.N);
                    ...
                    LogUtils.printLog(logger, "scheduleJob={}  PROGRESSING in {}...", scheduleJob.getId(), otherTaskTimeout);
                    return null;
                }
...
//                schemeJobLogDao.addOrUpdateSchemeJobLog(schemeJobLog);
            ...
            System.out.println("2" + schemeJobLog.getId());
            return jobLog;
        }
