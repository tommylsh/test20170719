CREATE  procedure [dbo].[udsp_convert_update_by_batch_IT19]
@runtype		bit, 
...
insert into it19.sales.dbo.runlog select runno, getdate(), brid, 'citem', count(*), 0, 
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
/*
insert into it19.sales.dbo.chklog select * from daily_chklog(nolock)
if @@ERROR <> 0 goto error_all

insert into it19.sales.dbo.runlog select runno, getdate(), brid, 'chklog', count(*), 0, 
...
--Carl 20140722
select a.* into #daily_cake_event_rcd_table from cake_event_disc_coupon_payment_table a (nolock), 
	(select distinct business_date, bran_code from daily_sales_order (nolock)) b
	where a.business_date = b.business_date and 'm' + a.order_branch_code = b.bran_code

delete it19.sales.dbo.cake_event_disc_coupon_payment_table from it19.sales.dbo.cake_event_disc_coupon_payment_table a, 
	(select distinct event_no from #daily_cake_event_rcd_table) b where a.event_no = b.event_no

insert into it19.sales.dbo.cake_event_disc_coupon_payment_table select * from #daily_cake_event_rcd_table
if @@ERROR <> 0 goto error_all
--Carl 20140722


------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
insert into it19.sales.dbo.coupon_order select * from daily_coupon_order(nolock)
if @@ERROR <> 0 goto error_all

insert into it19.sales.dbo.runlog select runno, getdate(), brid, 'cord', count(*), sum(total_amount), 
...
insert into it19.sales.dbo.coupon_tran select * from daily_coupon_tran(nolock)
if @@ERROR <> 0 goto error_all

insert into it19.sales.dbo.runlog select runno, getdate(), brid, 'ctrn', count(*), 0, 
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
/*
insert into it19.sales.dbo.damage select * from daily_damage(nolock)
if @@ERROR <> 0 goto error_all

insert into it19.sales.dbo.runlog select runno, getdate(), brid, 'damage', count(*), sum(itemtot), 
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
insert into it19.sales.dbo.sales_payfig select * from daily_sales_payfig
if @@ERROR <> 0 goto error_all

insert into it19.sales.dbo.runlog select runno, getdate(), bran_code, 'payfig', count(*), sum(total_amount), 
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--insert into view_insert_it19sale_order select * from view_daily_onhouse
--if @@ERROR <> 0 goto error_all
--
--insert into it19.sales.dbo.runlog select runno, getdate(), brid, 'onhouse', count(*), sum(foodamt), 
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--insert into it19.sales.dbo.recall select * from daily_recall(nolock)
--if @@ERROR <> 0 goto error_all

--insert into it19.sales.dbo.runlog select runno, getdate(), brid, 'recall', count(*), sum(itemtot), 
...
--insert into it19.sales.dbo.recall_salepay select * from daily_recall_salepay(nolock)
--if @@ERROR <> 0 goto error_all

--insert into it19.sales.dbo.runlog select runno, getdate(), brid, 'recpay', count(*), sum(payamt), 
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
insert into view_insert_it19sale_item select * From daily_sales_item
if @@ERROR <> 0 goto error_all


insert into it19.sales.dbo.runlog select runno, getdate(), bran_code, 'trans', count(*), sum(itemtot), 
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
insert into view_insert_it19sale_order select * from daily_sales_order
if @@ERROR <> 0 goto error_all

insert into it19.sales.dbo.runlog select runno, getdate(), bran_code, 'ords', count(*), sum(foodamt), 
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
insert into view_insert_it19payment select * from daily_sales_pay
if @@ERROR <> 0 goto error_all

insert into it19.sales.dbo.runlog select runno, getdate(), bran_code, 'salpay', count(*), sum(local_amount), 
...
insert into it19.sales.dbo.voidord select * from daily_voidord(nolock)
if @@ERROR <> 0 goto error_all

insert into it19.sales.dbo.runlog select runno, getdate(), brid, 'voidord', count(*), sum(foodamt), 
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
insert into it19.sales.dbo.voiditem select * from daily_voiditem(nolock)
if @@ERROR <> 0 goto error_all

insert into it19.sales.dbo.runlog select runno, getdate(), brid, 'voiditem', count(*), sum(itemtot), 
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
insert into it19.sales.dbo.sales_supp select * from daily_sales_supp(nolock)
if @@ERROR <> 0 goto error_all


insert into it19.sales.dbo.runlog select runno, getdate(), branch_code, 'supp', count(*), 0, 
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--insert into it19.sales.dbo.vip_reference select * from daily_vip_reference(nolock)
--if @@ERROR <> 0 goto error_all
--
--insert into it19.sales.dbo.runlog select runno, getdate(), brid, 'vip', count(*), sum(foodamt), 
...
insert into it19.sales.dbo.trans_ecard select * from daily_trans_ecard(nolock)
if @@ERROR <> 0 goto error_all


insert into it19.sales.dbo.runlog select runno, getdate(), branch_code, 'ecard', count(*), 0, 
...
while (@@FETCH_STATUS <> -1)
	begin
		select @xttdate = convert(char(9), @ttdate, 112)
		execute @r = it19.sales.dbo.udsp_delete_daily_sales_addition_detail @tempbranch, @xttdate
		fetch branchcursor into @tempbranch, @ttdate
	end
...
insert into it19.sales.dbo.daily_sales_addition_detail select * from daily_sales_addition_detail (nolock)
if @@ERROR <> 0 goto error_all
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
