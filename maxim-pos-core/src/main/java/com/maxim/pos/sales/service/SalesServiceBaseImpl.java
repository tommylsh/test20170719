...
//    protected @Value("${sales.lockTableInsert}")				boolean lockTableInsert;
//    protected @Value("${sales.lockTableThreshold}")				int lockTableThreshold;
...
//	        java.sql.Timestamp eodControlTimeoutTime = new java.sql.Timestamp(now.getTime().getTime() - eodTaskTimeout * 1000);
...
//                updateTaskJobLog(taskLog, start && prcDates != null);
            	taskJobLogService.updateTaskJobLogForEnd(taskLog, start && prcDates != null);
...
//                    BranchScheme branchSchemeToEdw = null;
                    if (branchSchemeToEdw != null) {
                        ...
                        String edwResult = null ;
                        try {
                        	edwResult = processStgToEdwJDBC(branchSchemeToEdw, controlDate, readyDates, taskLogEdw, logger);
                        } catch (Exception e) {
...
//                            updateTaskJobLog(taskLogEdw, true);
                            LogUtils.printLog(logger, "{} {} processPosDataToStg processStgToEdwJDBC {}", branchCode, pollSchemeType, edwResult);
                        	taskJobLogService.updateTaskJobLogForEnd(taskLogEdw, true);
                            if (edwResult == null)
                            {
                                throw new RuntimeException("processPosDataToStg processStgToEdwJDBC Failed !");
                            }
                        }
                    } else {
    ...
    public String processStgToEdwJDBC(BranchScheme branchScheme, java.sql.Date controlDate, List<Date> prcDate, TaskJobLog taskLogEdw, Logger logger) {
        if (branchScheme != null && branchScheme.isEnabled()) {
            ...
            LogUtils.printLog(logger, "{} {} processStgToEdwJDBC controlDate{} prcDates{}", branchScheme.getBranchMaster().getBranchCode(), branchScheme.getPollSchemeType(),
                    controlDate, prcDate);
            return processStgToEdwService.processStgToEdwJDBC(controlDate, prcDate);
        } else {
...
//    	
//        TaskJobLog currentTaskJobLog = taskJobLogDao.findByKey(taskJobLog.getId());
//
...
//                if (markFail)
//                {
//                	taskJobLogService.createJobExceptionDetail(taskJobLog,"","","Branch : " + taskJobLog.getBranchCode() + " Fail !! ");
//                    if (lastTaskJobLog != null) {
//                        if (!LatestJobInd.N.equals(lastTaskJobLog.getLastestJobInd())) {
//                            lastTaskJobLog.setLastestJobInd(LatestJobInd.N);
//                            taskJobLogDao.save(lastTaskJobLog);
