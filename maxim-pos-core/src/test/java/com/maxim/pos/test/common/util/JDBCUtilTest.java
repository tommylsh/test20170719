...
//		        "databaseName=hopos;user=sa;password=P@ssw0rd"; 
		...
		String toDS = "jdbc:jtds:sqlserver://10.20.30.166:1433/test;user=esb;password=esb123"; 
...
//			        String[] conditions =  new String[]{" branch_code  in ('2863','2833','4466')"};
			        String[] conditions =  new String[]{" BRANCH_CODE ='4459' "," status<> 'C' "};

			        
//					schemeInfo.setSrcKeyColumns("branch_code,business_date");
//					schemeInfo.setDestKeyColumns("branch_code,business_date");
					schemeInfo.setSrcKeyColumns("branch_code,item_code");
					schemeInfo.setDestKeyColumns("branch_code,item_code");
					 
					System.out.println("structureConsistentBulkCopy");
					JDBCUtils.jdbcBatchInsertFromResultSet(from, to, schemeInfo, 1000, conditions, true, true, JDBCUtils.CONV_NONE  );
...
//			try(Connection toConn = DriverManager.getConnection(oracle)){
			try(Connection toConn = applicationSettingService.getEdwJDBCConnection()){
				
				
				toConn.setAutoCommit(false);
				
				String[] criteria2 = new String[]{"branch_code='3409'"};
